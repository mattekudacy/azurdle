...
import { isFutureDate, todayUtc } from "@/lib/date";
...
export async function GET(request: NextRequest) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || isFutureDate(date)) {
    return NextResponse.json({ error: "invalid date" }, { status: 400 });
  }

  const admin = createAdminClient();

  // Community totals for this puzzle
  ...
  const totalPlayed = allRows?.length ?? 0;
  const totalSolved = allRows?.filter((r) => r.solved).length ?? 0;
  ...
  for (const row of allRows?.filter((r) => r.solved) ?? []) {
    solveDistribution[row.clues_revealed] = (solveDistribution[row.clues_revealed] ?? 0) + 1;
  }

  // Leaderboard: solvers ranked by clues → time
  const { data: solverRows } = await admin
    .from("attempts")
  ...
  // Current user's result for this puzzle
  ...
  try {
    ...
    // Non-fatal — anon users get no personal highlight
  }
